...
<div class="w-full">
    <div class="flex h-[91px] justify-center bg-[#1A1D23]">
        ...
        foreach ($schedule_of_movie as $item){
            ...
            <a href='index.php?act=movie-detail&id-movie=$id_movie&date=$schedule_date' class='block p-0'>
    ...
    foreach ($list_time as $item){
        ...
        echo "<div class='bg-[#1A1D23]'>
                         <div class='mx-auto w-full max-w-4xl py-6 space-y-6 relative '>
                           ...
                           <button class='flex items-center justify-center rounded-full text-sm font-medium border border-white hover:text-gray-300 hover:bg-orange-700 h-10 px-8 py-2'>
                             <p class='font-bold text-sm text-orange-500'>".$result['hour'].":".$result['minute']."</p>
                            </button>
                            </div>
                         </div>
                        </div>";
